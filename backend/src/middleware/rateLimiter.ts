...
export const rateLimitMiddleware = async (
  ...
  try {
    ...
    const maxConnectionsPerSecond = parseInt(process.env.CONNECTION_RATE_LIMIT_MAX || '100');
    ...
    if (!rateLimit.allowed) {
      ...
      console.warn(`Connection rate limit exceeded for ${ip}. Retry after ${retryAfter}ms`);
