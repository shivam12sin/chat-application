...
export class MessageRepository {
  ...
  static async getMessagesByRoom(
    ...
    if (cursor) {
      ...
      const cursorResult = await Database.query(`SELECT created_at FROM messages WHERE id = $1`, [
        cursor,
      ]);
  ...
  static async getMessageById(messageId: string): Promise<Message | null> {
    const result = await Database.query(`SELECT * FROM messages WHERE id = $1`, [messageId]);
   ...
   */
  static async createMessageReceipts(messageId: string, recipientIds: number[]): Promise<void> {
    if (recipientIds.length === 0) return;

    const values = recipientIds.map((_id, idx) => `($1, $${idx + 2})`).join(', ');
   ...
   */
  static async markMessagesAsDelivered(messageIds: string[], userId: number): Promise<void> {
  ...
  static async pinMessage(messageId: string): Promise<void> {
    await Database.query(`UPDATE messages SET is_pinned = TRUE WHERE id = $1`, [messageId]);
  }
  ...
  static async unpinMessage(messageId: string): Promise<void> {
    await Database.query(`UPDATE messages SET is_pinned = FALSE WHERE id = $1`, [messageId]);
  }
