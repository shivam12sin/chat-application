...
export class E2EKeyRepository {
  ...
  static async hasRegisteredKeys(userId: number): Promise<boolean> {
    const result = await Database.query('SELECT 1 FROM user_keys WHERE user_id = $1 LIMIT 1', [
      userId,
    ]);
   ...
   */
  static async consumeOneTimePrekey(
    userId: number,
  ...
  static async removeDevice(userId: number, deviceId: string): Promise<void> {
    await Database.query('DELETE FROM device_keys WHERE user_id = $1 AND device_id = $2', [
      userId,
      deviceId,
    ]);
  }
  ...
  static async deleteRoomSenderKeys(roomId: number): Promise<void> {
    await Database.query('DELETE FROM group_sender_keys WHERE room_id = $1', [roomId]);
  }
  ...
  static async getBackup(
    userId: number
  ): Promise<{ backupData: string; version: number; updatedAt: Date } | null> {
    const result = await Database.query(
