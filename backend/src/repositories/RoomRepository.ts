...
export class RoomRepository {
  ...
  static async getRoomById(roomId: number): Promise<Room | null> {
    const result = await Database.query(`SELECT * FROM rooms WHERE id = $1`, [roomId]);
   ...
   */
  static async setMemberAlias(roomId: number, userId: number, alias: string | null): Promise<void> {
    await Database.query(
      `UPDATE room_members 
             SET alias = $1
   ...
   */
  static async setChatLock(roomId: number, userId: number, locked: boolean): Promise<void> {
    await Database.query(
      `UPDATE room_members 
             SET is_locked = $1
             WHERE room_id = $2 AND user_id = $3 AND left_at IS NULL`,
