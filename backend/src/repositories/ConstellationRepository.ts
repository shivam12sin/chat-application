...
export class ConstellationRepository {
   ...
   */
  static async getUserConstellations(
    userId: number
  ): Promise<(Constellation & { message_count: number })[]> {
    const result = await Database.query(
      `SELECT c.*, 
   ...
   */
  static async getConstellationById(
    userId: number,
   ...
   */
  static async getConstellationMessages(userId: number, constellationId: number): Promise<any[]> {
    // Verify ownership
    const constellation = await this.getConstellationById(userId, constellationId);
