...
export class MessageDeleteRepository {
  ...
  static async unhideForUser(messageId: string, userId: number): Promise<void> {
    await Database.query(`DELETE FROM message_hidden_for WHERE message_id = $1 AND user_id = $2`, [
      messageId,
      userId,
    ]);
  }
  ...
  static async scheduleDelete(
    ...
    await Database.query(`UPDATE messages SET deleted_at = $2 WHERE id = $1`, [
      messageId,
      new Date(),
    ]);
  ...
  static async undoDelete(undoToken: string, requesterId: number): Promise<boolean> {
    ...
    // Restore message
    await Database.query(`UPDATE messages SET deleted_at = NULL WHERE id = $1`, [messageId]);

    // Remove from pending deletes
    await Database.query(`DELETE FROM pending_deletes WHERE id = $1`, [undoToken]);
  ...
  static async getPendingDelete(undoToken: string): Promise<PendingDelete | null> {
    const result = await Database.query(`SELECT * FROM pending_deletes WHERE id = $1`, [undoToken]);
  ...
  static async hardDeleteMessage(messageId: string): Promise<void> {
    ...
    await Database.query(`DELETE FROM pending_deletes WHERE message_id = $1`, [messageId]);
  ...
  static async getExpiredPendingDeletes(): Promise<PendingDelete[]> {
    const result = await Database.query(`SELECT * FROM pending_deletes WHERE delete_at <= NOW()`);
    return result.rows;
  }
