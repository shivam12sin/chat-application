...
router.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    const result = await Database.query('SELECT * FROM users WHERE email = $1 OR username = $1', [
      email,
    ]);
...
router.post('/2fa/login/verify', async (req: Request, res: Response) => {
  try {
    ...
    if (method === 'totp') {
      ...
      if (
        user.two_factor_secret_expires_at &&
        new Date() > new Date(user.two_factor_secret_expires_at)
      ) {
