...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      await E2EKeyRepository.storeSession(userId, peerUserId, sessionBuffer, roomId);
 ...
 */
router.get(
  '/devices/link-requests/:requestId/status',
  authenticateTokenHTTP,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user!.userId;
 ...
 */
router.post(
  '/devices/link-requests/:requestId/respond',
  authenticateTokenHTTP,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user!.userId;
