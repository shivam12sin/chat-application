...
class RoomHandler {
   ...
   */
  async handleJoinRoom(socket: AuthenticatedSocket, data: JoinRoomData): Promise<void> {
    try {
   ...
   */
  async handleLeaveRoom(socket: AuthenticatedSocket, data: LeaveRoomData): Promise<void> {
    try {
      const { userId, username } = socket;
      const { roomId } = data;
  ...
  async handleSetMemberAlias(
    ...
    try {
      ...
      socket
        .to(`room:${spaceId}`)
        .emit('space:alias_updated', { spaceId, userId, alias: trimmedAlias, username });
