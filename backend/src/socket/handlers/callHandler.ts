...
const ringingCalls = new Map<
  number,
  { timeout: NodeJS.Timeout; callerId: number; calleeId: number }
>();
...
class CallHandler {
  ...
  async handleInitiateCall(
    ...
    data: { calleeId: number; callType: 'voice' | 'video'; roomId?: number }
  ) {
    try {
  ...
  async handleEndCall(
    socket: AuthenticatedSocket,
    data: { callId: number; otherUserId: number; duration?: number }
  ) {
    const { userId } = socket;
    ...
    try {
      const { rows } = await Database.query('SELECT username FROM users WHERE id = $1', [
        otherUserId,
      ]);
      if (rows.length > 0) {
        await presenceHandler.broadcastPresence(socket, 'online', {
  ...
  async handleInviteToCall(
    ...
    const roomChannel = `call:room:${roomId}`;
    const sockets = await this.io?.in(roomChannel).fetchSockets();
    const isAlreadyIn = sockets?.some(
  ...
  handleSignal(
    socket: AuthenticatedSocket,
    data: { targetUserId: number; type: 'offer' | 'answer' | 'ice-candidate'; payload: any }
  ) {
    const { userId } = socket;
