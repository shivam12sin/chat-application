...
class MessageHandler {
  ...
  async handleSendMessage(
    ...
    try {
      ...
      const rateLimit = await RedisService.checkRateLimit(`ratelimit:message:${userId}`, 10, 1000);
   ...
   */
  private async queueForOfflineUsers(messageId: string, recipientIds: number[]): Promise<void> {
    try {
