...
class PresenceHandler {
  ...
  async handleDisconnection(socket: AuthenticatedSocket, reason: string): Promise<void> {
    try {
      ...
      // Remove session from database
      await Database.query(`DELETE FROM user_sessions WHERE socket_id = $1`, [socket.id]);

      // Update online status in Redis
      await RedisService.setUserOffline(userId, socket.id);

      // Update user's last_seen
      await Database.query(`UPDATE users SET last_seen = CURRENT_TIMESTAMP WHERE id = $1`, [
        userId,
      ]);
  ...
  public async broadcastPresence(
    socket:
      | AuthenticatedSocket
      | { userId: number; username: string; to: (room: string) => any }
      | { to: (room: string) => any },
