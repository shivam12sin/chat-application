...
export async function prepareOutgoingMessage(
  ...
  const recipientHasE2E =
    await e2eCryptoService.isUserE2EEnabled(recipientUserId);
  if (!recipientHasE2E) {
    console.log(
      `Recipient ${recipientUserId} does not have E2E enabled, sending plaintext`,
    );
    return { payload: message, isEncrypted: false };
  }
 ...
 */
export async function getConversationE2EStatus(peerUserId: number): Promise<{
  bothHaveE2E: boolean;
  selfHasE2E: boolean;
  peerHasE2E: boolean;
  safetyNumber?: string;
  ...
  const peerHasE2E = selfHasE2E
    ? await e2eCryptoService.isUserE2EEnabled(peerUserId)
    : false;
...
export async function handleGroupMemberJoined(
  ...
  try {
    await groupE2EService.handleMemberJoined(
      roomId.toString(),
      newMemberUserId,
    );
  } catch (error) {
