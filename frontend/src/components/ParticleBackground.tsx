  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      particles.current.forEach((p) => {
        ...
        // 1. Movement Logic
        const speedMultiplier = isAwakening.current
          ? AWAKEN_SPEED_MULTIPLIER
          : 1;
        ...
        if (p.type === "star") {
          p.opacity =
            p.baseOpacity + Math.sin(time * p.pulseSpeed * 100) * 0.05;
      ...
      particles.current = particles.current.filter(
        (p) => p.type === "star" || (p.life !== undefined && p.life > 0),
      );
