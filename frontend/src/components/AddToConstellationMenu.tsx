...
const AddToConstellationMenu: React.FC<AddToConstellationMenuProps> = ({
  ...
  const handleCreateConstellation = () => {
    ...
    socketService.createConstellation(newName.trim(), undefined, (response) => {
      ...
      if (response.constellation) {
        ...
        socketService.addToConstellation(
          response.constellation.id,
          messageId,
          roomId,
          () => {
            onSuccess?.();
  ...
  const handleToggleConstellation = (constellationId: number) => {
    ...
    if (isInConstellation) {
      socketService.removeFromConstellation(
        constellationId,
        messageId,
        (response) => {
          if (response.success) {
            setMessageConstellations((prev) =>
              prev.filter((id) => id !== constellationId),
            );
          }
        },
      );
          ...
          {loading ? (
            ...
            <div className="p-2">
              ...
              {constellations.map((constellation) => {
                const isSelected = messageConstellations.includes(
                  constellation.id,
                );
                return (
                  <button
          ...
          {showCreate ? (
            <div className="flex gap-2">
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && handleCreateConstellation()
                }
              />
              <button
