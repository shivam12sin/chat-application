...
const SpaceSettingsModal: React.FC<SpaceSettingsModalProps> = ({
  ...
  const [quietStart, setQuietStart] = useState(
    space.settings?.quietHours?.start || "22:00",
  );
  const [quietEnd, setQuietEnd] = useState(
    space.settings?.quietHours?.end || "08:00",
  );
  ...
  const handleSearchUsers = async () => {
    ...
    try {
      ...
      setSearchResults(
        res.data.filter((u: any) => !existingIds.includes(u.id)),
      );
    } catch {
          ...
          <div className="p-4 max-h-[60vh] overflow-y-auto">
            {activeTab === "general" && (
              <div className="space-y-4">
                <div>
                  <label className="block text-xs text-mono-muted mb-1">
                    Name
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs text-mono-muted mb-2">
                    Tone
                  </label>
                ...
                <div className="pt-4 border-t border-mono-glass-border">
                  ...
                  <p className="text-xs text-mono-muted/70 mb-2">
                    Set a custom tag that appears below your name in messages
                  </p>
              ...
              <div className="space-y-4">
                ...
                {searchResults.length > 0 && (
                  <div className="space-y-2 p-2 rounded-lg bg-mono-surface/50">
                    ...
                    {searchResults.map((user) => (
                      <div
                        key={user.id}
                        className="flex items-center justify-between p-2 rounded-lg hover:bg-mono-surface"
                      >
                ...
                <div className="space-y-2">
                  ...
                  {members.map((member) => (
                    <div
                      key={member.user_id}
                      className="flex items-center justify-between p-2 rounded-lg hover:bg-mono-surface/50"
                    >
                      <div className="flex items-center gap-2">
                        <Avatar
            ...
            {activeTab === "quiet" && (
              <div className="space-y-4">
                <p className="text-sm text-mono-muted">
                  During quiet hours, notifications from this space will be
                  muted and the UI will be dimmed.
                </p>
