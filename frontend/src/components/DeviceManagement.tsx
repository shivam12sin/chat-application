...
const CodeExpirationTimer: React.FC<{ expiresAt: Date }> = ({ expiresAt }) => {
  ...
  return (
    <p
      className={`text-xs mt-2 font-mono ${timeLeft === "Expired" ? "text-red-400" : "text-blue-400"}`}
    >
      Expires in: {timeLeft}
    </p>
  );
...
const DeviceList: React.FC = () => {
  ...
  const handleRemove = async (deviceId: string) => {
    if (
      !confirm(
        "Are you sure you want to remove this device? It will need to be re-linked to use E2E encryption.",
      )
    ) {
      return;
    }
            ...
            <div className="flex items-center gap-4">
              <div className="flex-shrink-0">
                ...
                {device.platform === "android" && (
                  <svg
                    className="w-8 h-8 text-green-400/80"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
              ...
              <div>
                ...
                <div
                  className="text-[10px] text-white/30 font-mono mt-1 w-32 truncate"
                  title={device.identityKeyFingerprint}
                >
      ...
      {mode === "create" ? (
        <div className="space-y-3">
          ...
          <div className="bg-blue-500/10 p-3 rounded-lg border border-blue-500/20 mb-3">
            <h4 className="text-blue-300 text-xs font-semibold uppercase tracking-wider mb-1">
              Why do I need this?
            </h4>
            <p className="text-blue-100/80 text-xs leading-relaxed">
              Your identity keys are now{" "}
              <strong>synced to the cloud (encrypted)</strong>. Create a backup
              to enable restoration on new devices.
            </p>
          </div>
            ...
            <div className="p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg text-sm text-yellow-300 flex justify-between items-center">
              <div>
                ...
                <div className="text-xs opacity-70">
                  {importedBackup.timestamp
                    ? new Date(importedBackup.timestamp).toLocaleString()
                    : "Unknown Date"}
                </div>
              </div>
              <button
...
export const DeviceManagement: React.FC<DeviceManagementProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<"devices" | "link" | "backup">(
    "devices",
  );
