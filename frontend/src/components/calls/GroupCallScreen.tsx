    ...
    <div className="fixed inset-0 z-[70] bg-zinc-950 flex flex-col overflow-hidden">
      ...
      <div className="absolute top-6 left-6 z-10 flex flex-col gap-2">
        <div className="px-3 py-1 bg-black/40 backdrop-blur rounded text-xs text-zinc-400 font-mono border border-white/5 flex items-center gap-2">
          <div
            className={`w-2 h-2 rounded-full ${dStatus === "Connected" ? "bg-green-500" : "bg-yellow-500"}`}
          />
      ...
      {/* Grid Container */}
      <div
        className={`flex-1 grid ${getGridClass(totalCount)} gap-4 p-4 md:p-8 overflow-y-auto`}
      >
        {/* Local User */}
        ...
        {participantList.map((p) => (
          <div
            key={p.userId}
            className="relative bg-zinc-900 rounded-2xl overflow-hidden border border-white/10 shadow-lg min-h-[200px]"
          >
            {p.stream ? (
