  ...
  useEffect(() => {
    ...
    const search = async () => {
      ...
      try {
        ...
        setResults(tracks);
        if (tracks.length === 0)
          setError("No results found. Ensure Spotify is configured on server.");
      } catch (err: any) {
        console.error("Search failed", err);
        if (err.message?.includes("not configured")) {
          setError(
            "Spotify not configured on server. Add SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET to backend .env",
          );
        } else {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-b border-mono-glass-border flex items-center gap-3">
        <Music2
          className={`w-6 h-6 text-mono-highlight ${isSearching ? "animate-pulse" : ""}`}
        />
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
        ...
        {!isSearching && error && (
          <div className="text-center text-red-400 py-10 text-sm bg-red-500/10 rounded-xl m-2 border border-red-500/20">
            {error}
          </div>
        )}
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  <h3
                    className="text-sm font-semibold text-mono-text truncate"
                    title={track.name}
                  >
                    {track.name}
                  </h3>
