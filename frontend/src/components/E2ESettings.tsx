  ...
  return (
    <div className={`glass rounded-2xl p-6 ${className}`}>
      <div className="flex items-center gap-3 mb-4">
        <div
          className={`p-2 rounded-xl ${isEnabled ? "bg-green-500/20" : "bg-gray-500/20"}`}
        >
          <svg
            className={`w-6 h-6 ${isEnabled ? "text-green-400" : "text-gray-400"}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
      ...
      <p className="text-white/70 text-sm mb-4">
        ...
        communicating with can read your messages. Not even we can read them.
      </p>
      ...
      {status && isEnabled && (
        <div className="glass-subtle rounded-xl p-4 mb-4 space-y-2">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span
              className={`${status.needsPrekeyRefill ? "text-yellow-400" : "text-white/80"}`}
            >
          ...
          {status.signedPrekeyAge && (
            <div className="flex justify-between text-sm">
              ...
              <span
                className={`${status.needsSignedPrekeyRotation ? "text-yellow-400" : "text-white/80"}`}
              >
                {Math.floor(status.signedPrekeyAge / (24 * 60 * 60 * 1000))}{" "}
                days
              </span>
            </div>
          )}
