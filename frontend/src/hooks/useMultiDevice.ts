...
import {
  ...
  type DeviceInfo,
  type KeyBackup,
} from "../crypto/MultiDeviceService";
  ...
  const generateCode = useCallback(async () => {
    ...
    try {
      const { code, expiresAt } =
        await multiDeviceService.generateLinkingCode();
      setLinkingCode(code);
      setCodeExpiresAt(expiresAt);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Failed to generate linking code",
      );
    } finally {
      setLoading(false);
    }
...
export function useKeyBackup(): UseKeyBackupReturn {
  ...
  const restoreBackup = useCallback(
    async (password: string, backupToRestore?: KeyBackup): Promise<boolean> => {
      setLoading(true);
      setError(null);

      try {
        // Note: Update service to accept password first
        const success = await multiDeviceService.restoreKeyBackup(
          password,
          backupToRestore,
        );
