...
export function useE2E(_options: UseE2EOptions = {}): UseE2EReturn {
  ...
  const initialize = useCallback(
    async (config: E2EConfig) => {
      ...
      try {
        ...
        const message =
          err instanceof Error ? err.message : "Failed to initialize E2E";
        setError(message);
        throw err;
      } finally {
        setIsLoading(false);
  ...
  const enable = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to enable E2E";
      setError(message);
      throw err;
    } finally {
      setIsLoading(false);
  ...
  const disable = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to disable E2E";
      setError(message);
      throw err;
    } finally {
      setIsLoading(false);
...
export function E2EStatusBadge({
  peerUserId,
  className = "",
}: E2EStatusBadgeProps): JSX.Element | null {
  const { bothHaveE2E, selfHasE2E, peerHasE2E, isLoading } =
    useConversationE2E(peerUserId);
  ...
  if (selfHasE2E && !peerHasE2E) {
    return (
      <div
        ...
        title="Peer has not enabled E2E encryption"
      >
        <svg
          className="w-4 h-4"
          fill="none"
