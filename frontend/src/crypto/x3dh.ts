...
export async function x3dhInitiate(
  ...
  const recipientIdentityKey = await importEd25519PublicKey(
    recipientKeyBundle.identityPublicKey,
  );
  ...
  if (recipientKeyBundle.oneTimePreKey) {
    const oneTimePreKeyPublic = await importX25519PublicKey(
      recipientKeyBundle.oneTimePreKey.publicKey,
    );
...
export async function x3dhRespond(
  ...
  if (recipientOneTimePreKey && header.recipientOneTimePreKeyId !== undefined) {
    const oneTimePreKeyPrivate = await importX25519PrivateKey(
      recipientOneTimePreKey.privateKey,
    );
...
export function deserializeX3DHHeader(data: Uint8Array): X3DHHeader {
  ...
  return {
    ...
    recipientOneTimePreKeyId: hasOneTimePreKey
      ? view.getUint32(68, false)
      : undefined,
  };
}
