...
async function openDatabase(): Promise<IDBDatabase> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(new Error("Failed to open E2E key database"));
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!database.objectStoreNames.contains(STORES.SIGNED_PREKEYS)) {
        const store = database.createObjectStore(STORES.SIGNED_PREKEYS, {
          keyPath: "keyId",
        });
      ...
      if (!database.objectStoreNames.contains(STORES.ONE_TIME_PREKEYS)) {
        const store = database.createObjectStore(STORES.ONE_TIME_PREKEYS, {
          keyPath: "keyId",
        });
      ...
      if (!database.objectStoreNames.contains(STORES.SESSIONS)) {
        const store = database.createObjectStore(STORES.SESSIONS, {
          keyPath: "sessionKey",
        });
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = async () => {
      if (request.result) {
        ...
        putRequest.onerror = () =>
          reject(new Error("Failed to store device key"));
...
export async function storeIdentityKeyPair(
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(new Error("Failed to store identity key pair"));
...
export async function getIdentityKeyPair(): Promise<IdentityKeyPair | null> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(new Error("Failed to get identity key pair"));
...
export async function getCurrentSignedPreKey(): Promise<SignedPreKey | null> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(new Error("Failed to get current signed prekey"));
...
export async function storeOneTimePreKeys(
  ...
  return new Promise((resolve, reject) => {
    ...
    for (const preKey of preKeys) {
      ...
      encryptForStorage(new TextEncoder().encode(serialized)).then(
        (encrypted) => {
          const request = store.put({
            keyId: preKey.keyId,
            data: encrypted,
          ...
          request.onerror = () => {
            ...
            if (completed + errors === preKeys.length) {
              if (errors > 0)
                reject(new Error(`Failed to store ${errors} prekeys`));
              else resolve();
            }
          };
...
export async function markOneTimePreKeyUsed(keyId: number): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(new Error("Failed to delete one-time prekey"));
    request.onsuccess = () => resolve();
  });
}
...
export async function countUnusedOneTimePreKeys(): Promise<number> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(new Error("Failed to count one-time prekeys"));
...
export async function getHighestPreKeyId(): Promise<number> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(new Error("Failed to get highest prekey ID"));
...
export async function getAllSessions(): Promise<
  Array<{ peerUserId: number; roomId?: number; state: SessionState }>
> {
  const database = await openDatabase();
