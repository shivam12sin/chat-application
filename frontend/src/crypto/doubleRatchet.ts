...
async function dhRatchet(
  ...
  if (state.dhSendingKeyPair) {
    const dhPrivate = await importX25519PrivateKey(
    ...
    const [newRootKey, receivingChainKey] = await kdfRootKey(
      state.rootKey,
      dhOutput,
    );
  ...
  const [newRootKey, sendingChainKey] = await kdfRootKey(
    state.rootKey,
    dhOutput,
  );
  state.rootKey = newRootKey;
...
export async function decrypt(
  ...
  const currentDhKeyBase64 = state.dhReceivingKey
    ? toBase64(state.dhReceivingKey)
    : null;
...
async function skipMessageKeys(
  ...
  const dhKeyBase64 = state.dhReceivingKey
    ? toBase64(state.dhReceivingKey)
    : "";
...
async function tryDecryptWithSkippedKeys(
  ...
  const index = state.skippedMessageKeys.findIndex(
    (sk) =>
      sk.dhPublicKey === dhKeyBase64 &&
      sk.messageIndex === message.header.messageIndex,
  );
...
export function serializeSessionState(state: SessionState): string {
  return JSON.stringify({
    ...
    sendingChainKey: state.sendingChainKey
      ? toBase64(state.sendingChainKey)
      : null,
