...
import { toBase64, fromBase64, concat, generateFingerprint } from "./utils";
...
class E2ECryptoService {
  ...
  async getStatus(): Promise<E2EStatus> {
    ...
    return {
      ...
      signedPrekeyAge,
      needsSignedPrekeyRotation: signedPrekeyAge
        ? signedPrekeyAge > 7 * 24 * 60 * 60 * 1000
        : false,
    };
  }
   ...
   */
  private async fetchKeyBundle(
    userId: number,
  ): Promise<ReceivedKeyBundle | null> {
    if (!this.config) throw new Error("E2E service not initialized");
  ...
  private async getOrCreateSession(
    ...
    const x3dhHeader = serializeX3DHHeader(
      createX3DHHeader(
        this.identityKeyPair.publicKey,
  ...
  private async handleX3DHHeader(
    ...
    if (header.recipientOneTimePreKeyId !== undefined) {
      oneTimePreKey =
        (await getOneTimePreKey(header.recipientOneTimePreKeyId)) ?? undefined;
      if (oneTimePreKey) {
  ...
  async encryptMessage(
    ...
    const { session, isNew, x3dhHeader } = await this.getOrCreateSession(
      peerUserId,
      roomId,
    );
  ...
  async decryptMessage(
    ...
    const cacheKey = this.getSessionCacheKey(peerUserId, roomId);
    let session =
      this.sessionCache.get(cacheKey) || (await getSession(peerUserId, roomId));
    ...
    if (payload.isInitialMessage && payload.x3dhHeader) {
      ...
      session = await this.handleX3DHHeader(
        peerUserId,
        x3dhHeaderBytes,
        roomId,
      );
