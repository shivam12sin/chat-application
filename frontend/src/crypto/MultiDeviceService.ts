...
class MultiDeviceService {
  ...
  async registerDevice(): Promise<DeviceRegistrationData> {
    ...
    await axios.post(`${this.config.apiUrl}/e2e/devices`, registrationData, {
  ...
  private getPlatformFromName(
    ...
    if (lower.includes("electron") || lower.includes("desktop_app"))
      return "desktop";
    return "web";
  }
  ...
  async getDevices(): Promise<DeviceInfo[]> {
    ...
    return devices.map((d) => ({
      ...
      isCurrentDevice:
        ...
        (this.deviceId || "").replace(/-/g, "").toLowerCase(),
      ...
      identityKeyFingerprint: d.identityPublicKey
        ? d.identityPublicKey
            .substring(0, 16)
            .match(/.{1,4}/g)
            ?.join(" ") || d.identityPublicKey.substring(0, 16)
        : "Unknown",
  ...
  async removeDevice(deviceId: string): Promise<void> {
    ...
    await axios.delete(`${this.config.apiUrl}/e2e/devices/${deviceId}`, {
      headers: { Authorization: `Bearer ${this.config.token}` },
    });
  ...
  async checkCloudBackup(): Promise<boolean> {
    ...
    try {
      await axios.get(`${this.config.apiUrl}/e2e/backup`, {
        headers: { Authorization: `Bearer ${this.config.token}` },
      });
      return true;
    } catch (error) {
  ...
  async fetchCloudBackup(): Promise<KeyBackup | null> {
    ...
    try {
      const response = await axios.get(`${this.config.apiUrl}/e2e/backup`, {
        headers: { Authorization: `Bearer ${this.config.token}` },
      });
