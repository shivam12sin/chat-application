...
class GroupE2EService {
  ...
  private receivedSenderKeys: Map<string, Map<number, SenderKeyRecord>> =
    new Map();
  ...
  private async loadStoredKeys(): Promise<void> {
    ...
    for (const key of stateKeys) {
      try {
        ...
        if (stateJson) {
          this.senderKeyStates.set(
            roomId,
            deserializeSenderKeyState(stateJson),
          );
        }
      } catch (error) {
    ...
    for (const key of recordKeys) {
      try {
        ...
        if (recordJson) {
          ...
          this.receivedSenderKeys
            .get(roomId)!
            .set(senderId, deserializeSenderKeyRecord(recordJson));
        }
      } catch (error) {
  ...
  async distributeSenderKey(
    ...
    for (const memberId of memberUserIds) {
      ...
      try {
        ...
        console.error(
          `Failed to distribute sender key to user ${memberId}:`,
          error,
        );
      }
    }
  ...
  async processSenderKeyDistribution(
    ...
    try {
      ...
      const distribution: SenderKeyDistributionMessage =
      ...
      console.log(
        `Received sender key from user ${senderId} for room ${roomId}`,
      );
    } catch (error) {
  ...
  getMissingSenderKeyMembers(roomId: string, memberIds: number[]): number[] {
    ...
    return memberIds.filter(
      (id) => id !== this.config!.userId && !roomKeys.has(id),
    );
  }
  ...
  isGroupE2EAvailable(roomId: string, memberIds: number[]): boolean {
    ...
    return (
      this.senderKeyStates.has(roomId) &&
      this.hasAllSenderKeys(roomId, memberIds)
    );
  }
