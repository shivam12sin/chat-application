...
const SpaceSettingsModal = React.lazy(
  () => import("../components/SpaceSettingsModal"),
);
const PinnedMessagesDrawer = React.lazy(
  () => import("../components/PinnedMessagesDrawer"),
);
const AddToConstellationMenu = React.lazy(
  () => import("../components/AddToConstellationMenu"),
);
const GroupCallScreen = React.lazy(
  () => import("../components/calls/GroupCallScreen"),
);
const IncomingCallModal = React.lazy(
...
const isWithinQuietHours = (settings?: {
...
function Home() {
  ...
  const [lockTargetRoom, setLockTargetRoom] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const [activeCallType, setActiveCallType] = useState<"voice" | "video">(
    "voice",
  );
  ...
  const [lastDeletedMessage, setLastDeletedMessage] = useState<Message | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const fetchBlockedUsers = async () => {
      try {
        ...
        setBlockedUserIds(
          response.data.map((b: { blocked_id: number }) => b.blocked_id),
        );
      } catch (err) {
    ...
    const fetchMessages = async () => {
      ...
      try {
        const response = await axios.get(
          `${API_URL}/messages/room/${selectedRoomId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        ...
        prev.map((room) => {
          if (room.id === message.room_id) {
            ...
            if (message.sender.id !== currentUser?.id) {
              ...
              const isQuiet =
                room.room_type === "group" && isQuietHoursActive(room.settings);
  ...
  useEffect(() => {
    ...
    if (
      prevId &&
      unlockedSessionRooms.includes(prevId) &&
      prevId !== currentId
    ) {
        ...
        prev.map((msg) => {
          if (msg.id === messageId) {
            ...
            if (existingReaction) {
              ...
              newReactions = [
                ...existingReactions,
                { emoji, count: 1, by: [currentUser?.username || ""] },
              ];
            }
      ...
      if (response.error) {
        ...
        if (locked) {
          ...
          setLockedRoomIds((prev) =>
            prev.filter((id) => id !== selectedRoomId),
          );
          setUnlockedSessionRooms((prev) =>
            prev.filter((id) => id !== selectedRoomId),
          );
    ...
    return (
      <div className="h-screen text-mono-text flex items-center justify-center flex-col gap-4">
        <div className="flex gap-1">
          {[0, 1, 2].map((i) => (
            <div key={i} className="w-3 h-3 rounded-full bg-mono-muted/40" />
          ))}
        </div>
        <p className="text-mono-muted">Connecting...</p>
        <ChromeButton onClick={logout} className="mt-4 px-4 py-2 text-sm">
          Cancel
        </ChromeButton>
      </div>
    );
      ...
      <div
        className={cn(
          ...
          currentRoom?.room_type === "group" &&
            isWithinQuietHours(currentRoom?.settings) &&
          ...
          {currentRoom && (
            <div className="flex gap-2 flex-shrink-0">
              ...
              <RoomOptionsMenu
                ...
                isMuted={
                  selectedRoomId ? mutedRoomIds.includes(selectedRoomId) : false
                }
                isLocked={
                  selectedRoomId
                    ? lockedRoomIds.includes(selectedRoomId)
                    : false
                }
                token={token!}
                onMuteChange={(muted) => {
                  ...
                  if (currentRoom?.other_user_id) {
                    if (blocked) {
                      setBlockedUserIds((prev) => [
                        ...prev,
                        currentRoom.other_user_id!,
                      ]);
                    } else {
                      setBlockedUserIds((prev) =>
                        prev.filter((id) => id !== currentRoom.other_user_id),
                      );
                    }
                  }
                }}
              ...
              <ChatSearch
                ...
                onNavigateToMessage={(messageId) => {
                  ...
                  if (element) {
                    ...
                    setTimeout(
                      () => element.classList.remove("bg-mono-surface-2/50"),
                      2000,
                    );
                  }
                }}
      ...
      <Suspense fallback={null}>
        {currentRoom && currentRoom.room_type === "group" && (
          <SpaceSettingsModal
            ...
            onSpaceUpdated={(updatedSpace) => {
              setRooms((prev) =>
                prev.map((r) =>
                  r.id === updatedSpace.id ? { ...r, ...updatedSpace } : r,
                ),
              );
            }}
            onSpaceLeft={() => {
        ...
        <ForwardModal
          ...
          onForward={async (roomIds) => {
            ...
            success(
              `Forwarded to ${roomIds.length} chat${roomIds.length > 1 ? "s" : ""}`,
            );
